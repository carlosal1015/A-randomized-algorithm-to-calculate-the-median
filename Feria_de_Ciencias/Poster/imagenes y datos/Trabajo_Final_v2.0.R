  ...
  ################################################
    ...
    tam=c(500*1:100)     #mucho más ordenado y equitativo que el anterior
    ...
    abline(lsfit(tam,y1),col="blue") #arriba
    abline(lsfit(tam,y2),col="red") #debajo
    abline(lsfit(tam,y3),col="green") #constante
    
    
    #Se agrega una leyenda para entender el grafico
    legend(x=3,y=11, legend=c("QS", "RQS","median de R"), lty=c(17,16,18),col=c("blue", "red","green"))
    ...
    library(readxl)
    Libro1 <- read_excel("C:/Users/ASUS/Desktop/Libro1.xlsx")
    View(Libro1)
    
    
    #plot(Libro1$x,Libro1$s,pch=17,col="blue",xlab="Tamaño de datos",ylab="Tiempo total(s)",ylim=c(0,4),main="Grafico QS vs R-QS")
    points(Libro1$x,Libro1$s,pch=13,col="red")
    points(Libro1$x,Libro1$qs,pch=14,col="red")
    points(Libro1$x,Libro1$r,pch=16,col="green")
    
    qua1 <- lm(y2~Libro1$x)
    summary(qua1)
    legend(y=4, legend=c("QS", "RQS","median de R"), lty=c(17,16,18),col=c("blue", "red","green"))
    
    
    qua2 <- lm(y1~tam)
    summary(qua2)
    
    abc <- c(Libro1$x[11],Libro1$x[21])
    abcy <- c(Libro1$qs[11],Libro1$qs[21])
    
    abline(lsfit(Libro1$x,Libro1$s),col="blue") #arriba
    abline(lsfit(Libro1$x,Libro1$qs),col="red") #debajo
    abline(lsfit(Libro1$x,Libro1$r),col="green") #constante
    lines(Libro1$qs~Libro1$x,col="blue")
    lines(c(Libro1$x[1],Libro1$x[10]),c(Libro1$qs[1],Libro1$qs[10]),col="blue")
    
    plot(tam,y3,pch=17,col="yellow",xlab="Tamaño de datos",ylab="Tiempo total(s)",main="Grafico QS vs R-QS")
    points(Libro1$x,Libro1$s,pch=13,col="red")
    points(Libro1$x,Libro1$qs,pch=14,col="blue")
    points(Libro1$x,Libro1$r,pch=16,col="green")